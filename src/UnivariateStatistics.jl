...
"""
    std(A::UnivariateStatistic; corrected=false)
Compute the sample standard deviation of a `A`, from its variance (uncorrected by default).

"""
Statistics.std(A::UnivariateStatistic; corrected=false) = sqrt(var(A; corrected))
